...
// TODO uhhhhh.......
// global var bad
backend_knows_we_joined_as_client = false;
    ...
    while (PluginPacketGetBuffer(Contracts.packetID) != -1) {
        ...
        switch (header) {
            ...
            case Contracts.NET_GAME_SRV_UPDATE_CONTRACTS:
                ...
                count = read_ubyte(buf);
                for (i = 0; i < count; i += 1) {
                    // TODO refactor to merge that code with the part that does the same thing but from the backend
                    var newContract;
                    newContract = instance_create(0, 0, Contracts.Contract);
                    with (newContract) {
                        contract_id = read_binstring(buf, 16);
                        contract_type = read_ubyte(buf);
                        target_value = read_ubyte(buf);
